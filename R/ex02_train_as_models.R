...
reg = batchtools::makeExperimentRegistry(file.dir = "DLTSP-AS-Experiments",
  packages = c("tidyverse", "BBmisc", "checkmate", "mlr", "salesperson", "parallel", "ParamHelpers", "kernlab", "randomForest", "xgboost", "rpart"))

##################################################################################################

dynamic_fun = function(data, job, feature.sets, tsp.sets, solver.sets) {
...
classif = function(data, job, instance, learner, s, meth) {
  ...
  if (meth != "none") {
    ...
    filename = sprintf("%s/intermediate_as_results/sf_classif/sf_classif__%s__%s__%s__%s__%s__%i.RData",
      instance$wd, instance$feature.sets.short, instance$tsp.sets.short, instance$solver.sets.short, learner, meth, s)
  ...
  filename = sprintf("%s/intermediate_as_results/as_classif/classif__%s__%s__%s__%s__%s__%i.RData",
    instance$wd, instance$feature.sets.short, instance$tsp.sets.short, instance$solver.sets.short, learner, meth, s)
  if (meth == "none") {
    sf = NULL
  }
...
ids.ga1 = findExperiments(algo.pars = (meth == "GA1" & learner != "xgboost"), prob.pars = (tsp.sets == "sophisticated" & feature.sets == "salesperson"))
ids.ga1.xg = findExperiments(algo.pars = (meth == "GA1" & learner == "xgboost"), prob.pars = (tsp.sets == "sophisticated" & feature.sets == "salesperson"))

ids.ga2 = findExperiments(algo.pars = (meth == "GA2" & learner != "xgboost"), prob.pars = (tsp.sets == "sophisticated" & feature.sets == "salesperson"))
